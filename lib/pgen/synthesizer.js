  ...
  class Prod {
    ...
    getParser(lexNames) {
      ...
      function getParsers(tokens, cond, idx, isSub) {
        ...
        let argStrList = params? params : [];
        parser = `${parser} ~ ${subName}(${argStrList.join(', ')}) ^^ { case `;
      ...
      let paramStrList = params? params.map(p => p + ': Boolean') : [];
      let paramStr = '(' + paramStrList.join(', ' ) + ')'

      code += `  def ${name}${paramStr}: NodeParser[ast.${name}] = {` + '\n';
      code += main;
      code += `    MISMATCH` + '\n';
      code += `  }` + '\n';
      code += `  def sub${name}${paramStr}: NodeParser[ast.${name} => ast.${name}] = {` + '\n';
      code += sub;
      code += `    MATCH ^^^ { x => x }` + '\n';
      code += `  }` + '\n';
      return code;
  ...
  class Rhs {
    ...
    static from(elem) {
      ...
      if (condAttr != null) switch (condAttr[0]) {
        case '+': cond = condAttr.substr(1); break;
        case '~': cond = '!' + condAttr.substr(1); break;
      }
...
`;
  ...
  return {
    ...
    get ES2018() {
      ...
      let spaceNames = new Set(['WhiteSpace', 'LineTerminator', 'LineTerminatorSequence'])
      removedElems = [];
