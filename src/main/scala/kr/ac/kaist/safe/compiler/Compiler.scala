...
class Compiler(program: ast.Program) {
  ...
  private def walkProgram(
    ...
    val ast.Program(_, ast.TopLevel(_, fds, vds, sts)) = pgm
    fds.foreach(walkFd(_, insts))
    vds.foreach(walkVd(_, insts))
    sts.foreach(_.body.foreach(walkStmt(_, insts)))
  }
  ...
  private def walkVd(
    ...
    val ast.VarDecl(_, id, expr, _) = vd
    expr.map(_ => excLog.signal(VarDeclNotHaveInitExprError(vd)))
    core.IConst(newId(id), core.Undef)
  }
  ...
  //
  // // default strings
  // private var locals: List[String] = List[String]()
  ...
  // private val TRUE_BOOL = core.Bool(true)
  // private val FALSE_BOOL = core.Bool(false)

  // // reference error
  // private lazy val REF_ERROR = NU.REF_ERR_NAME

  // // global temporal id
  // private lazy val GLOBAL_TMP_ID = NU.GLOBAL_NAME
